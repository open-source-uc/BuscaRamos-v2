...
export default function EquivCoursesSection({
  equivalences = [],
  hasEquivalences,
  className = "",
  loading = false,
}: Props) {
  if (!hasEquivalences || equivalences.length === 0) {
    return (
          ...
          <CollapsibleContent className="border-border bg-accent data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-up-1 data-[state=open]:slide-down-1 w-full overflow-hidden border-t px-6 py-4">
            {loading ? (
              <div className="space-y-2 py-6">
                {Array.from({ length: Math.max(equivalences.length, 1) }).map((_, i) => (
                  <Skeleton key={i} className="h-10 w-full rounded-md" />
                ))}
              </div>
            ) : (
              <div className="w-full overflow-hidden">
                <EquivCoursesDisplay equivalences={equivalences} />
              </div>
            )}
          </CollapsibleContent>
        </Collapsible>
